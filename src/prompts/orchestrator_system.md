...
1. `run_usecases`: Takes a plain text string of usecases or a filepath to a usecase file, executes browser automation on samsung.com, and extracts the chatbot's actual responses.
2. `evaluate_responses`: Uses the Evaluation sub-agent to compare the actual chatbot responses against the expected responses.
